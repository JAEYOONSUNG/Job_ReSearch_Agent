...
def discover_by_topic() -> dict:
    ...
    # Build search queries from CV keywords (combine related ones)
    queries: list[str] = []
    for kw in CV_KEYWORDS:
        # Wrap multi-word keywords in quotes for exact phrase matching
        if " " in kw:
            q = f'"{kw}"[Title/Abstract] AND {date_filter}'
        else:
            q = f"{kw}[Title/Abstract] AND {date_filter}"
        queries.append(q)

    logger.info("Running %d PubMed topic queries", len(queries))
    ...
    for query in queries:
        ...
        for batch_start in range(0, len(pmids), 50):
            ...
            for article in articles:
                ...
                if article.get("year"):
                    # Store the keyword that matched in fields
                    pi_record["fields"] = query.split("[")[0].strip().strip('"')
